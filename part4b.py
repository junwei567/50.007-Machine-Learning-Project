from constants import ES_TRAIN_DATA_FILE, ES_TEST_DATA_FILE, ES_ACTUAL_DATA_FILE, ES_PART4_PREDICTED_DATA_FILE, RU_TRAIN_DATA_FILE, RU_TEST_DATA_FILE, RU_ACTUAL_DATA_FILE, RU_PART4_PREDICTED_DATA_FILE, START_TAG, STOP_TAG, UNK_WORD
from evaluate import evaluateScores, write_to_predicted_file
from preprocessing import _preprocess_training_file2, _preprocess_test_file2, _preprocess_training_file1
...
def get_transition_pairs_using_MLE(unique_tags, transition_pair_count, tags_with_start_stop):
...
def get_transition_triplets_using_MLE(unique_tags, transition_triplet_count, tags_with_start_stop):
    transition = {}
    unique_tags = [START_TAG] + unique_tags + [STOP_TAG]
...
if __name__ == "__main__":
    ...
    # Get ES tags
    ES_tags, ES_tags_with_start_stop, ES_train_words = _preprocess_training_file2(ES_TRAIN_DATA_FILE)
    ...
    ES_transition_pair_parameters = get_transition_pairs_using_MLE(ES_unique_tags, ES_transition_pair_count, ES_tags_with_start_stop)
    ES_transition_triplet_count = get_transition_triplets(ES_tags_with_start_stop)
    ES_transition_triplet_parameters = get_transition_triplets_using_MLE(ES_unique_tags, ES_transition_triplet_count, ES_tags_with_start_stop)
    ...
    # Get RU tags
    RU_tags, RU_tags_with_start_stop, RU_train_words = _preprocess_training_file2(RU_TRAIN_DATA_FILE)
    ...
    RU_transition_pair_parameters = get_transition_pairs_using_MLE(RU_unique_tags, RU_transition_pair_count, RU_tags_with_start_stop)
    RU_transition_triplet_count = get_transition_triplets(RU_tags_with_start_stop)
    RU_transition_triplet_parameters = get_transition_triplets_using_MLE(RU_unique_tags, RU_transition_triplet_count, RU_tags_with_start_stop)
    ...
    for word in ES_test_words:
        viterbi_values = {}
        start_viterbi(word, ES_unique_words, ES_unique_tags, ES_emission_parameters, ES_transition_pair_parameters, ES_transition_triplet_parameters)
        ES_generated_tag_list = generate_predictions_viterbi(word, ES_unique_tags, ES_transition_triplet_parameters)
        ES_predicted_tags_list.append(ES_generated_tag_list)

    write_to_predicted_file(ES_PART4_PREDICTED_DATA_FILE, ES_test_words, ES_predicted_tags_list)
    ...
    for word in RU_test_words:
        viterbi_values = {}
        start_viterbi(word, RU_unique_words, RU_unique_tags, RU_emission_parameters, RU_transition_pair_parameters, RU_transition_triplet_parameters)
        RU_generated_tag_list = generate_predictions_viterbi(word, RU_unique_tags, RU_transition_triplet_parameters)
        RU_predicted_tags_list.append(RU_generated_tag_list)

    write_to_predicted_file(RU_PART4_PREDICTED_DATA_FILE, RU_test_words, RU_predicted_tags_list)
