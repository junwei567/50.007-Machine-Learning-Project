from constants import ES_TRAIN_DATA_FILE, ES_TEST_DATA_FILE, ES_ACTUAL_DATA_FILE, ES_PART3_PREDICTED_DATA_FILE, RU_TRAIN_DATA_FILE, RU_TEST_DATA_FILE, RU_ACTUAL_DATA_FILE, RU_PART3_PREDICTED_DATA_FILE, START_TAG, STOP_TAG, UNK_WORD
...
def generate_predictions_viterbi(word_list, tags_unique, transmission_params):
    ...
    for i in range(n-1, 0, -1):
        ...
        for tags in total_viterbi_scores.values():
            ...
            for tag in tags_unique:
                try:
                    value = viterbi_values[(i, tag)] + math.log(transmission_params[tag][tags[0]]) # we take the first tag because we are working backwards
                    link[value] = [tag] + tags
                except ValueError:
                    continue
...
if __name__ == "__main__":
    ...
    write_to_predicted_file(ES_PART3_PREDICTED_DATA_FILE, ES_test_words, ES_predicted_tags_list)
    ...
    write_to_predicted_file(RU_PART3_PREDICTED_DATA_FILE, RU_test_words, RU_predicted_tags_list)
